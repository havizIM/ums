<div class="row pt-2 pb-2">
    <div class="col-sm-12">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#/approval_cuti">Approval Cuti</a></li>
            <li class="breadcrumb-item active" aria-current="page">Detail Approval - Cuti</li>
        </ol>
    </div>
 </div>
 ...
 <script>
    ...
    function render_content(data){
        var content = '';
            ...
            if((data.pemohon.id_divisi !== auth.id_divisi && data.status === 'Approve 2') || (data.pemohon.id_divisi === auth.id_divisi && data.status === 'Approve 1')){
                        ...
                        <div class="col-md-8">
                            <div class="alert alert-icon-info alert-dismissible" role="alert">
                                ...
                                <span>
                                    ...
                                    Dengan mengapprove Data Cuti ini maka anda menyetujui karyawan yang bersangkutan cuti. 
                                </span>
                                </div>
                            </div>
                            <div class="card">
    ...
    function load_data(id, render_content){
        ...
        $.ajax({
            url: `<?= base_url('api/approval_cuti/show/') ?>${auth.token}?id=${id}`,
            type: 'GET',
            dataType: 'JSON',
            beforeSend: function(){
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '#tolak_cuti', function(){
            ...
            swal({
                ...
                if (isConfirm) {
                $.ajax({
                    url: `<?= base_url('api/approval_cuti/tolak/') ?>${auth.token}?id=${id}`,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(response){
        ...
        $(document).on('click', '#approve_cuti', function(){
            ...
            swal({
                ...
                if (isConfirm) {
                $.ajax({
                    url: `<?= base_url('api/approval_cuti/approve_3/') ?>${auth.token}?id=${id}`,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(response){
                        if(response.status === 200){
