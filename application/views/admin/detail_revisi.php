<!-- Breadcrumb-->
 <div class="row pt-2 pb-2">
    <div class="col-sm-9">
        <h4 class="page-title">Detail - Koreksi Absen</h4>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#/dashboard">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="#/revisi_absen">Pengajuan</a></li>
            <li class="breadcrumb-item active" aria-current="page">Detail Koreksi Absen</li>
        </ol>
    </div>
 </div>
 ...
 <script>
            ...
            if(data.status === 'Proses'){
                content += `<div class="text-center">
                                <div class="btn-group group-round">
                                    <button class="btn btn-sm btn-danger" id="batal_revisi" data-id="${data.id}"><i class="fa fa-close"></i> Batalkan</button>
                                    <a class="btn btn-sm btn-success" href="#/edit_revisi/${data.id}"><i class="fa fa-pencil"></i> Ubah</a>
                                </div>
                            </div>`;
    ...
    $(document).ready(function(){
        var id = location.hash.substr(15);

        load_data(id, render_content);

        $(document).on('click', '#batal_revisi', function(){
            ...
            swal({
                ...
                if (isConfirm) {
                $.ajax({
                    url: `<?= base_url('api/revisi_absen/batalkan/') ?>${auth.token}?id=${id}`,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(response){
